...
### Backend
- [ ] **BE-01**: 모노레포 셋업 (`apps/`, `packages/`, `infra/`, `docs/`) | 부록 C | S | `make setup` 한 번에 의존성 설치, README에 시작 가이드
- [ ] **BE-02**: FastAPI 프로젝트 구조 + `/api/v1/health` 엔드포인트 | 부록 G | S | 200 응답, OpenAPI 자동 생성 활성화
- [ ] **BE-03**: Alembic 베이스라인 마이그레이션 | 부록 E | S | `alembic upgrade head` 통과
- [ ] **BE-04**: SSO/OIDC 인증 미들웨어 (Authlib) + 로컬 mock IdP | §F7, 부록 C | L | 로컬에서 로그인 e2e, JWT 발급, refresh token

### Frontend
- [ ] **FE-01**: Next.js (App Router) + TypeScript + Tailwind + shadcn/ui 셋업 | 부록 C | M | 빌드 통과, Storybook 셋업
- [ ] **FE-02**: 디자인 토큰 정리 (색·타이포·간격), 다크모드 토글 | M | 토큰 페이지에서 시각 확인
- [ ] **FE-03**: 인증 플로 (SSO 리다이렉트, 세션 쿠키, 401 처리) | §F7 | M | 로그인-로그아웃 e2e
...
### Backend
- [ ] **BE-05**: 부록 E의 8개 테이블 마이그레이션 (User, Project, AnalysisSession, Finding, FindingStatus, Comment, Agent, Preset) | 부록 E | M
- [ ] **BE-06**: SQLAlchemy 2.0 async 모델 + Pydantic v2 스키마 | 부록 E, F | M
- [ ] **BE-07**: 부록 F의 Agent ABC + 데이터 스키마 (AgentMetadata, CodeScope, Preset, Finding 등) 구현 | 부록 F | M
- [ ] **BE-08**: MockAgent 구현 (랜덤 finding 생성, 결정론적 시나리오) | 부록 F | M | `MockAgent().analyze()`가 5건 mock finding 반환
- [ ] **BE-09**: Entry-point 기반 에이전트 디스커버리 | 부록 F | S | `pyproject.toml` entry_points로 등록 → 부팅 시 자동 발견
- [ ] **BE-10**: Project CRUD 엔드포인트 (`GET/POST /projects`, `GET/PATCH /projects/{id}`) | 부록 G | M
- [ ] **BE-11**: User CRUD + RBAC 데코레이터 베이스 (Reviewer/Lead/Admin) | §F7 | M

### Frontend
- [ ] **FE-04**: 프로젝트 목록 페이지 (필터: 상태, 담당) | F1 | M
- [ ] **FE-05**: 프로젝트 상세 페이지 — 개요 탭 (와이어프레임 기준) | 와이어프레임 #1 | M
- [ ] **FE-06**: 프로젝트 신규 등록 폼 | F1-1, F1-2 | M
- [ ] **FE-07**: 디자인 시스템 컴포넌트 (Card, Badge, StatCard, Button, Input) | M
...
### Backend
- [ ] **BE-12**: GitLab API 클라이언트 (인증, 프로젝트/브랜치 검색) | F2-1, F2-2 | M
- [ ] **BE-13**: 코드 적재 서비스 (격리 환경에 클론) — 우선 임시 디렉토리, K8s Job은 Sprint 3 | F2-4 | L
- [ ] **BE-14**: AnalysisSession CRUD + 상태 전이 검증 (`QUEUED → PREPARING → RUNNING → ...`) | 부록 A | M
- [ ] **BE-15**: 새 분석 회차 시작 엔드포인트 (`POST /projects/{id}/sessions`) — 우선 동기 실행 (Mock) | 부록 G | M
- [ ] **BE-16**: 세션 로그 SSE 엔드포인트 | 부록 G | M

### Frontend
- [ ] **FE-08**: 프로젝트 상세 — 회차 탭 + 새 분석 실행 모달 | F1, F3 | L
- [ ] **FE-09**: GitLab 브랜치 선택 컴포넌트 (자동완성) | F2-2 | M
- [ ] **FE-10**: 회차 진행 화면 (SSE 로그 스트림 표시) | F3-3 | M
...
### Backend
- [ ] **BE-17**: Temporal Activity 6종 구현 (부록 H) — Mock 버전 | 부록 H | L
- [ ] **BE-18**: AnalysisWorkflow 본 코드 + 단위 테스트 | 부록 H | L
- [ ] **BE-19**: K8s Job 프로비저닝 Activity (실제 Pod 생성) | §C.6 | L
- [ ] **BE-20**: Finding fingerprint 산출 로직 | §B.2 | M
- [ ] **BE-21**: Finding API (`GET /sessions/{id}/findings`, `GET /findings/{id}`) | 부록 G | M
- [ ] **BE-22**: Finding 상태 변경 (`PATCH /findings/{id}/status`) + FindingStatus append | 부록 G | M

### Frontend
- [ ] **FE-11**: 검증 화면 골격 (좌: Monaco 코드 뷰어, 우: 사이드 패널) | 와이어프레임 #2 | L
- [ ] **FE-12**: 취약점 리스트 + 인라인 코드 점프 | 부록 D | L
- [ ] **FE-13**: 검증 액션 버튼 (확정/오탐/검토) + 낙관적 업데이트 | 부록 D | M
- [ ] **FE-14**: 단축키 핸들러 (j/k/c/f/r/e/?) | §D.3 | S
...
### Backend
- [ ] **BE-23**: ClaudeCodeAgent 구현 (Anthropic API + Claude Code CLI 격리 실행) | 부록 F | L
- [ ] **BE-24**: CodexAgent 구현 (OpenAI API + Codex CLI 격리 실행) | 부록 F | L
- [ ] **BE-25**: 토큰 사용량/비용 추적 — `LogEvent.tokens_used` 누적, 회차 종료 시 집계 | §A.5 | M
- [ ] **BE-26**: ResourceLimits 강제 (시간/토큰/비용 한도 초과 시 자체 중단) | §A.5 | M
- [ ] **BE-27**: 프리셋 CRUD + 빌트인 프리셋 3종 (표준 보안 검수, Quick Diff Scan, PII 집중) | §A.3 | M

### Frontend
- [ ] **FE-15**: 분석 실행 모달 — 에이전트 선택 + 프리셋 선택 + 스코프 (브랜치/diff/include path) | F3-1, F3-2 | M
- [ ] **FE-16**: 회차 상세 — 메타데이터 표시 (에이전트, 모델 버전, 토큰, 비용) | §A.1 | S
- [ ] **FE-17**: 프리셋 관리 페이지 (목록 + 편집) | F3-2 | M
...
### Backend
- [ ] **BE-28**: Comment CRUD (`/findings/{id}/comments`) | 부록 G | M
- [ ] **BE-29**: 동시 편집 충돌 감지 (낙관적 락 또는 last-writer-wins + 알림) | §D.4 | M
- [ ] **BE-30**: WebSocket 핸들러 — 검증 화면 실시간 동기화 | §D.4 | M
- [ ] **BE-31**: 리포트 생성 (PDF, Markdown, CSV, JSON) | §B.3 | L
- [ ] **BE-32**: `POST /sessions/{id}/reports` 엔드포인트 | 부록 G | M

### Frontend
- [ ] **FE-18**: 코멘트 UI (작성, 표시, mention) | F4-4 | M
- [ ] **FE-19**: 동시 편집 충돌 UI (다른 검수자가 변경 시 표시) | §D.4 | M
- [ ] **FE-20**: 리포트 내보내기 모달 (템플릿 선택, 포맷 선택) | §B.3 | M
- [ ] **FE-21**: 필터 바 (심각도, 카테고리, 상태) | F4-1 | M
...
### Backend
- [ ] **BE-33**: 회귀 매칭 알고리즘 (fingerprint 기반 NEW/RECURRING/RESOLVED/CARRIED_OVER 라벨링) | §B.2 | L
- [ ] **BE-34**: `GET /findings/{id}/timeline` (회귀 이력) | 부록 G | M
- [ ] **BE-35**: `GET /queue` (실행 중·대기 중 통합) | 부록 G | M
- [ ] **BE-36**: 작업 우선순위 큐 (URGENT/NORMAL/BACKGROUND) | §A.5 | M
- [ ] **BE-37**: 검색·필터 API (기간, 담당, 심각도, 상태) | F5-4 | M

### Frontend
- [ ] **FE-22**: 프로젝트 상세 — 이력 탭 (회차 비교) | F5-1 | M
- [ ] **FE-23**: 회귀 추적 타임라인 화면 | 와이어프레임 #4 | M
- [ ] **FE-24**: 분석 큐 모니터링 화면 | 와이어프레임 #3 | L
- [ ] **FE-25**: 실시간 큐 갱신 (5초 polling 또는 WebSocket) | §F6 | S
...
### Backend
- [ ] **BE-38**: 비용 집계 (일별/에이전트별/프로젝트별 materialized view) | 부록 I | M
- [ ] **BE-39**: 비용 가시화 API (`GET /usage/cost`, `/usage/by-project`, `/usage/by-agent`) | 부록 I | M
- [ ] **BE-40**: 권한 매트릭스 정식화 (역할 × 엔드포인트 표) + 미들웨어 적용 | §F7 | L
- [ ] **BE-41**: 감사 로그 sink (별도 PG 테이블 + 외부 SIEM 연계 인터페이스) | §F7 | M
- [ ] **BE-42**: 알림 시스템 (Slack 웹훅 + Email) — 트리거 4종 | §I.5 | M

### Frontend
- [ ] **FE-26**: 비용 가시화 화면 | 와이어프레임 #5 | L
- [ ] **FE-27**: 설정 — 사용자/권한 페이지 (Admin 전용) | §F7 | M
- [ ] **FE-28**: 설정 — 감사 로그 페이지 | §F7 | M
- [ ] **FE-29**: 설정 — GitLab 연동, 에이전트/프리셋 페이지 | §F7 | M
...
### Backend
- [ ] **BE-43**: 에러 처리 표준화 (Problem Details RFC 7807) | 부록 G | M
- [ ] **BE-44**: rate limiting (사용자/엔드포인트별) | M
- [ ] **BE-45**: API 문서 자동 생성 점검 + 외부 공유 가능 형태 정리 | 부록 G | S
- [ ] **BE-46**: 부하 테스트 (k6 또는 Locust, 100 동시 검수자 시뮬레이션) | §7.2 | L

### Frontend
- [ ] **FE-30**: 접근성 점검 (axe-core, 키보드 내비게이션) | M
- [ ] **FE-31**: 에러 바운더리 + 공통 에러 화면 | S
- [ ] **FE-32**: 사용자 프로필/설정 페이지 | S
- [ ] **FE-33**: 온보딩 모달 (첫 로그인 시 가이드) | M
