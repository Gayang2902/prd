...
class ClaudeCodeAgent(BaseAgent):
    ...
    def describe(cls) -> AgentMetadata:
        return AgentMetadata(
            ...
            cost_profile={"per_1k_input_tokens": 0.003, "per_1k_output_tokens": 0.015},
            description="Security analysis agent powered by Claude Code CLI via Anthropic API.",
        )

    async def prepare(self, context: AnalysisContext) -> None:
        self._api_key = os.environ.get("ANTHROPIC_API_KEY")
        if not self._api_key:
            secret = context.secrets.get("ANTHROPIC_API_KEY")
            if secret:
    ...
    async def analyze(self, context: AnalysisContext) -> AsyncIterator[LogEvent | AnalysisResult]:
        if not self._api_key:
            yield LogEvent(
                ...
                message="ANTHROPIC_API_KEY not configured. Cannot run Claude Code agent.",
            )
        ...
        yield LogEvent(
            timestamp=datetime.now(timezone.utc),
            level=LogLevel.INFO,
            message="Starting Claude Code security analysis",
            progress=0.0,
        )
        ...
        try:
            ...
            yield LogEvent(
                timestamp=datetime.now(timezone.utc),
                level=LogLevel.INFO,
                message="Claude Code analysis completed",
                progress=0.9,
            )
            ...
            yield LogEvent(
                timestamp=datetime.now(timezone.utc),
                level=LogLevel.ERROR,
                message=f"Claude Code timed out after {context.limits.max_runtime_seconds}s",
            )
