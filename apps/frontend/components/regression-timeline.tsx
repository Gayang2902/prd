...
export function RegressionTimeline({ projectId }: Props) {
  ...
  if (!history?.length) {
    return <p className="text-sm text-muted-foreground">완료된 분석 세션이 없습니다.</p>;
  }
        ...
        <Card key={entry.session_id} className="p-0">
          <CardHeader className="p-4 pb-2">
            <div className="flex items-center justify-between">
              <CardTitle className="font-mono text-xs">{entry.commit_sha.slice(0, 8)}</CardTitle>
          ...
          <CardContent className="p-4 pt-0 space-y-2">
            <div className="flex gap-2">
              <Badge variant="destructive" className="text-[10px]">NEW {entry.new}</Badge>
              <Badge variant="secondary" className="text-[10px]">RECURRING {entry.recurring}</Badge>
              <Badge variant="outline" className="text-[10px]">RESOLVED {entry.resolved}</Badge>
            </div>
