...
import { fetchCostSummary, fetchCostByProject, fetchCostByAgent, fetchDailyCost } from '@/lib/api/usage';
...
describe('usage API', () => {
  ...
  it('fetchCostSummary with date range', async () => {
    ...
    expect(mockApiFetch).toHaveBeenCalledWith('/usage/cost?since=2025-01-01&until=2025-01-31');
  });

  it('fetchCostByProject', async () => {
    mockApiFetch.mockResolvedValue([]);
    await fetchCostByProject('2025-01-01');
    expect(mockApiFetch).toHaveBeenCalledWith('/usage/by-project?since=2025-01-01');
  });
...
describe('audit API', () => {
  ...
  it('fetchAuditLogs with filters', async () => {
    ...
    expect(mockApiFetch).toHaveBeenCalledWith('/audit/logs?action=login&limit=50&offset=10');
  });

  it('fetchAuditLogs with resource_type', async () => {
    mockApiFetch.mockResolvedValue([]);
    await fetchAuditLogs({ resource_type: 'project' });
    expect(mockApiFetch).toHaveBeenCalledWith('/audit/logs?resource_type=project');
  });
});
...
describe('regression API', () => {
  it('fetchRegressionHistory', async () => {
    ...
    expect(mockApiFetch).toHaveBeenCalledWith('/projects/p1/regression-history');
