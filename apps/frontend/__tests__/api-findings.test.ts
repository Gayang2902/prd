...
import { fetchFindings, updateFindingStatus, fetchFindingTimeline } from '@/lib/api/findings';
...
describe('findings API', () => {
  ...
  it('fetchFindings with severity filter', async () => {
    ...
    expect(mockApiFetch).toHaveBeenCalledWith('/sessions/s1/findings?severity=critical');
  });
